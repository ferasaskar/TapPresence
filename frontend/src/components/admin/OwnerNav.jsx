...
import { Home, CreditCard, Inbox, CalendarDays, BarChart3, Settings as SettingsIcon, ScanLine, LogOut, ShieldCheck } from "lucide-react";
...
export const OwnerNav = ({ active }) => {
  ...
  const refresh = () => {
    ...
    api.get("/admin/leads").then((r) => setUnread(r.data.filter((l) => !l.read).length)).catch(() => {});
  };
  useEffect(() => { refresh(); }, []);
  const primary = cards[0];

  const items = [
    { key: "home", label: "Home", icon: Home, onClick: () => navigate("/dashboard") },
    { key: "cards", label: cards.length > 1 ? "My Cards" : "My Card", icon: CreditCard, onClick: () => navigate("/admin") },
    { key: "inbox", label: "Inbox", icon: Inbox, onClick: () => setLeadsOpen(true), badge: unread },
    { key: "meetings", label: "Meetings", icon: CalendarDays, onClick: () => navigate("/meetings") },
    { key: "analytics", label: "Analytics", icon: BarChart3, onClick: () => (primary ? setAnalyticsCard(primary) : navigate("/dashboard")) },
        ...
        <div className="flex items-center justify-between gap-3">
          <button onClick={() => navigate("/dashboard")} className="flex items-center gap-2.5" data-testid="nav-brand">
            ...
            <div className="text-left">
              ...
              <h1 className="text-[14px] font-medium leading-tight text-white">My Studio</h1>
            </div>
          </button>
          <div className="flex items-center gap-2">
            {isAdmin ? (
              <span className="hidden items-center gap-1.5 rounded-full border border-[#D6A653]/40 bg-[#D6A653]/10 px-3 py-1 text-[11px] font-medium text-[#D6A653] sm:inline-flex" data-testid="nav-role-badge"><ShieldCheck className="h-3.5 w-3.5" /> Super Admin · Global</span>
            ) : (
              <span className="hidden rounded-full border border-white/12 bg-white/5 px-3 py-1 text-[11px] text-white/55 sm:inline" data-testid="nav-role-badge">{user?.name || "Member"}</span>
            )}
            {isAdmin ? (
              <button onClick={() => setScanOpen(true)} className="rounded-full border border-white/12 bg-white/5 p-2 text-white/70 transition-colors hover:text-white" title="Scan card" data-testid="nav-scan"><ScanLine className="h-4 w-4 text-[#D6A653]" /></button>
            ) : null}
          ...
          {items.map((it) => {
            ...
            return (
              ...
              >
                ...
                {it.label}
                {it.badge > 0 ? <span className="ml-0.5 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-[#D6A653] px-1 text-[9px] font-semibold text-[#050607]" data-testid="nav-inbox-badge">{it.badge}</span> : null}
              </button>
            );
          })}
        </nav>
