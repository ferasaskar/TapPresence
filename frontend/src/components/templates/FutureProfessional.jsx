...
const Overline = ({ children }) => (
  <p className="text-[11px] uppercase tracking-[0.35em] mb-4 text-sky-300/80">{children}</p>
);
...
export const FutureProfessional = ({ data }) => {
  ...
  return (
    <div className="relative min-h-screen font-sans text-slate-200 overflow-hidden" style={{ backgroundColor: "#070A16" }}>
      <div className="grain-overlay" style={{ opacity: 0.05 }} />
      <div className="pointer-events-none absolute left-1/2 top-[-60px] h-[420px] w-[420px] -translate-x-1/2 rounded-full opacity-30 blur-3xl" style={{ background: "radial-gradient(circle, #6d5cff 0%, #1b8cff 55%, transparent 75%)" }} />
      ...
      <div className="relative mx-auto w-full max-w-lg px-6 pb-16 pt-14 sm:px-8">
        ...
        <motion.header {...fade(0)} className="flex flex-col items-center text-center">
          ...
          <div className={`mb-5 rounded-full px-5 py-2 ${glass}`}>
            <AvailabilityBadge
              ...
              className="inline-flex items-center gap-2 text-xs tracking-widest uppercase text-sky-300"
              dotClassName="w-1.5 h-1.5 rounded-full bg-sky-400 animate-pulse"
            />
          </div>
        ...
        <motion.div {...fade(1)} className={`mt-10 grid grid-cols-2 gap-2 rounded-2xl p-2 ${glass}`}>
          {panel.map((a) => (
            <ActionButton
              ...
              iconClassName="w-5 h-5 text-sky-300"
            />
          ))}
        </motion.div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {services.map((s, i) => {
                ...
                return (
                  ...
                  >
                    <div className="absolute inset-x-6 -bottom-px h-px opacity-0 transition-opacity duration-300 group-hover:opacity-100" style={{ background: "linear-gradient(90deg,transparent,#4aa8ff,transparent)" }} />
                    <span className="mb-5 inline-flex h-11 w-11 items-center justify-center rounded-xl" style={{ background: "rgba(74,168,255,0.12)" }}>
                      <Icon className="w-5 h-5 text-sky-300" strokeWidth={1.5} />
                    </span>
        ...
        {projects.length > 0 && (
          <section className="mt-16" data-testid="projects-section">
                  ...
                  <>
                    ...
                    <div className="flex items-center justify-between gap-3 p-5">
                      <div className="min-w-0">
                        <p className="text-[10px] uppercase tracking-[0.2em] mb-1 text-sky-300/80 truncate">{p.category}</p>
                      ...
                      <ArrowRight className="w-5 h-5 text-sky-300 transition-transform duration-300 group-hover:translate-x-1" strokeWidth={1.5} />
                    </div>
                  </>
                );
            ...
            <div className="mt-5 flex items-center justify-center gap-2" data-testid="slider-dots">
              {projects.map((_, i) => (
                <span key={i} className={`h-1.5 rounded-full transition-all ${i === 0 ? "w-6 bg-sky-400" : "w-1.5 bg-white/20"}`} />
              ))}
            </div>
          </section>
        )}
        ...
        {(b.bookingUrl || c.phone) && (
          <motion.section {...fade(0)} className="mt-16 relative" data-testid="cta-bar">
            <div className="absolute inset-0 rounded-full blur-2xl opacity-50" style={{ background: "linear-gradient(90deg,#6d5cff,#1b8cff)" }} />
            <ActionButton
        ...
        <style>{`[data-testid="cta-book-button"]{background:linear-gradient(90deg,#6d5cff,#2b8cff);box-shadow:0 8px 40px rgba(90,80,255,0.45);}`}</style>
        ...
        <motion.section {...fade(0)} className="mt-6 grid grid-cols-2 gap-4">
          <SaveContactButton
            ...
            iconClassName="w-6 h-6 text-sky-300"
            label="Save Contact"
        ...
        <footer className="mt-16 border-t border-white/10 pt-8 text-center">
          ...
          <SocialIcons
            ...
            itemClassName="flex items-center justify-center w-10 h-10 rounded-full border border-white/10 bg-white/[0.03] text-slate-400 transition-colors duration-300 hover:text-sky-300 hover:border-sky-400/50"
          />
